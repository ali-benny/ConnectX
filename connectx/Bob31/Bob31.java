...
public class Bob31 implements CXPlayer {
    ...
    public int selectColumn(CXBoard board) {
        ...
        return bestCol == -1 ? avlCol[0] : bestCol;
    }
    ...
    /**
     * The evaluate function is used to calculate a score for the current board state.
     * The higher the score, the better it is for this player.
     * 
     *
     * @param CXBoard board Get the board state
     *
    ...
    private int evaluate(CXBoard board) {
        if (board.gameState() == myWin) {
            ...
            for (int row = 0; row < board.M; row++) {
                CXCellState[] rowState = getRowState(board, row);
                for (int col = 0; col < board.N; col++) {
                    if (rowState[col] == CXCellState.FREE)
                        continue;
                    if (rowState[col] == CXCellState.P1 && myWin == CXGameState.WINP1)  // se la cella ha una pedina mia aggiungo del punteggio
                        score += searchTranspositionTable(board); //zobrist[row][col][1];
                    else if (rowState[col] == CXCellState.P2 && myWin == CXGameState.WINP2)
                        score += searchTranspositionTable(board); //zobrist[row][col][2];
                    else
                        score -= searchTranspositionTable(board); //zobrist[row][col][rowState[col]];          // se la cella ha una pedina avversaria sottraggo del punteggio
                }
            }
            return score;
        }
    ...
    public CXCellState[] getRowState(CXBoard board, int rowIndex) {
        CXCellState[] rowState = new CXCellState[board.N];
        for (int j = 0; j < board.N; j++) {
            rowState[j] = board.cellState(rowIndex, j);
        }
        return rowState;
    }
